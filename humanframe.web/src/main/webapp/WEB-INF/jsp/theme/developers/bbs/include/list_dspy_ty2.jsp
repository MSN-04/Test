	...
	<c:forEach items="${noticeListVO.listOutptObject}" var="listOutpt" varStatus="status">
		...
		<tr>
			...
			<c:if test="${item.value != 0 }">
				<img src="/comm/getImage?srvcId=BBSTY2&amp;upperNo=${mapHide.nttNo}&amp;fileTy=THUMBNAIL&amp;fileNo=${item.value}&amp;thumbTy=S" alt="<spring:message code='ui.thumbnail'/>" />
			</c:if>
			</c:when>
		...
		<tr>
			<c:forEach var="item" items="${mapShow}">
				...
				<c:choose>
					...
					<c:if test="${mapHide.nttOrdr > 1 }"><img src="${themeAssets}/img/icon/icon_reply.gif" alt="<spring:message code='board.reply' />" /></c:if>
