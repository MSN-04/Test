...
<%@ include file="../include/load_plugin.jsp" %>
	...
	$(document).ready(function(){
		...
		$("input[name='mvpTy']").on("click", function(){
			if($("input[name='mvpTy']:checked").val() == "1"){
				$(".mvp-html-tr").hide();
				$(".file-upload-tr").show();
			}else{
				$(".mvp-html-tr").show();
	...
	$(function(){
		$("#frmNtt").validate({
		    ...
		    messages : {
		    	...
		    	, thumbnailFile : { required : "<spring:message arguments='" + $("#thumbnailFile").prop("title") + "' code='errors.required'/>"}
		    },
	...
	function f_save(){
		...
		if($("input:checkbox[name=noticeAt]").is(":checked")){
			$("#thumbnailFile").rules("remove", "required");
		}
		else{
			$("#thumbnailFile").rules("add", {required : true});
		}
					...
					<!-- Start Selectable Table Row -->
						<table class="table table-bordered mb-none">
									...
									<c:choose>
										...
										<c:when test="${viewObj.key eq 'mvpTy' }"><!-- 동영상 유형 -->
											<c:forEach var="viewDtl" items="${viewObj.value }">
											<tr>
												<th>${viewDtl.key}<span class="required">*</span></th>
												<td>
													<div class="radio-group">
														<f:radiobuttons path="mvpTy" items="${mvpTyCode}" element="span class='radio-custom radio-inline radio-primary' " cssClass="form-control"/>
													</div>
												</td>
											</tr>
											</c:forEach>
										...
										<c:when test="${viewObj.key eq 'mvpHtml' }"><!-- 동영상 HTML -->
											<c:forEach var="viewDtl" items="${viewObj.value }">
											<tr class="mvp-html-tr" style="display: none;">
												<th>${viewDtl.key}</th>
												<td>
													<f:textarea path="mvpHtml" cssClass="form-control" style="width:100%;height:100px;" placeholder="${viewDtl.key} 입력" title="${viewDtl.key}"/>
												</td>
											</tr>
											</c:forEach>
										</c:when>
