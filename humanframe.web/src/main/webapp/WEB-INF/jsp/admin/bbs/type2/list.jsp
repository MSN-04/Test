			...
			<section class="panel panel-featured panel-featured-primary">
				<header class="panel-heading">
					<div class="panel-actions">
						<div class="btn-group">
							<button class="btn btn-default dropdown-toggle" data-toggle="dropdown">With Selected <span class="caret"></span></button>
	                    		...
	                    		<div class="form-inline ml-md">
	                    			<c:if test="${bbsSettingVO.ctgryUseAt eq 'Y'}">
										...
										<select id="srchKey" name="srchKey" class="form-control">
	                                		...
	                                		<c:if test="${bbsSettingVO.bbsNo eq 360 }">
		                                		<option value="deleteReson" <c:if test="${param.srchKey eq 'deleteReson'}">selected="selected"</c:if>>삭제사유</option>
		                                	</c:if>
	                                	</select>
	                                    <input type="text" id="srchText" name="srchText" value="${param.srchText}" class="form-control">
	                                    <input type="text" id="srchBeginDt" name="srchBeginDt" value="${param.srchBeginDt}" class="form-control" readonly="readonly" style="display: none;">
	                                    <input type="text" id="srchEndDt" name="srchEndDt" value="${param.srchEndDt}" class="form-control" readonly="readonly" style="display: none;">

	                                    <button type="submit" class="btn btn-default"><i class="fa fa-search"></i> 검색</button>
	                                <c:if test="${param.bbsNo eq '862'}" >
		                                    <button type="button" class="btn btn-default" onclick="javascript:f_excel(); "><i class="fa fa-file-excel-o"></i> 엑셀다운로드</button>
	                               	</c:if>
									...
									<c:forEach items="${listVO.listOutptObject}" var="listOutpt" varStatus="status">
										...
										<label></label>
											...
											<c:forEach var="item" items="${mapShow}">
												...
												<c:when test="${ item.key eq 'thumb' }">
												<c:if test="${item.value != 0 }">
													<img src="/comm/getImage?srvcId=BBSTY2&amp;upperNo=${mapHide.nttNo}&amp;fileTy=THUMBNAIL&amp;fileNo=${item.value}&amp;thumbTy=S" alt="썸네일이미지" />
												</c:if>
												</c:when>

												<c:when test="${ item.key eq 'sj' }">
												<c:if test="${mapHide.noticeAt eq 'Y'}"><img src="${globalAdminAssets}/images/icon/icon_notice.gif" alt="공지사항" style="margin-right: 5px;"/></c:if>
												<c:forEach begin="1" end="${mapHide.nttLevel}">&nbsp;&nbsp;&nbsp;&nbsp;</c:forEach>
												<c:if test="${mapHide.nttOrdr > 1 }"><img src="${globalAdminAssets}/images/icon/icon_reply.gif" alt="답글" /></c:if>
												...
												<c:if test="${listVO.paramMap.searchMode eq 'DELETE' and not empty mapHide.deleteReson}">
													(사유 : ${mapHide.deleteReson })
												</c:if>
												<c:if test="${mapHide.secretAt eq 'Y'}"><i class="iconfa-lock"></i></c:if>
												</c:when>
