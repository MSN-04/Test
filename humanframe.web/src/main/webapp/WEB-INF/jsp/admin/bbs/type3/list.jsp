					...
					<div class="tab-content">
						<form id="searchFrm" name="searchFrm" class="form-horizontal form-bordered mb-md" method="get" action="./list">
							...
							<div class="row mg-files" >
								<c:forEach items="${listVO.listObject}" var="imageList" varStatus="status">
								<c:set var="imgList" value="${cmsFn:getImageSrcs(imageList.cn)}"/>
										...
										<div class="thumbnail">
											<div class="thumb-preview">
												...
												<c:choose>
						                        	...
						                        	<c:otherwise>
						                        		<img src="/comm/getImage?srvcId=BBSTY3&amp;upperNo=${imageList.nttNo}&amp;fileTy=ATTACH&amp;fileNo=${imageList.fileNo}&amp;thumbTy=M" class="img-responsive" alt="썸네일이미지" style="width: 240px;height: 160px;"/>
						                        	</c:otherwise>
						                        </c:choose>
						                        </cmsBtn:view>
											</div>
											<h5 class="mg-title text-weight-semibold">
												...
												<input type="checkbox" data-style="checkbox" id="arrNttNo${status.index}" name="arrNttNo" value="${imageList.nttNo}" />
												<label></label>
												</span>
												${fn:substring(imageList.sj, 0, 20)}${fn:length(imageList.sj) > 20? '...':'' }
											</h5>
											<div class="mg-description">
												<small class="pull-left text-muted">${imageList.wrter}</small>
												<small class="pull-right text-muted">${cmsFn:convertDate(imageList.writngDe, 'yyyy.MM.dd') }</small>
											</div>
										</div>
									</div>
								</c:forEach>
