...
<%@ include file="/common/taglibs.jsp" %>
	...
	function f_excel(){
		$("#excelFrm").attr("action", "/admin/bbs/type1/excel");
	...
	<input type="hidden" name="bbsNo" id="bbsNo" value="${bbsSettingVO.bbsNo}" />
									...
									<tbody>
										...
										<c:forEach items="${noticeListVO.listOutptObject}" var="listOutpt" varStatus="status">
											...
											<label></label>
												...
												<c:if test="${bbsSettingVO.reportAt eq 'Y' }">
												<td><a href="javascript:f_report_view('${mapHide.bbsNo }','${mapHide.nttNo }');">${mapHide.reportCnt}</a></td>
												</c:if>
											</tr>
										</c:forEach>
										<!-- 공지사항 게시물 끝 -->
		...
		<jsp:include page="../include/load_report.jsp" />