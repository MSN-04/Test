	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src","/humanframe/global/assets/smarteditor/img/photoQuickPopup/btn_confirm2.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
	    	 ...
	    	 welBtnConfirm.attr("src","/humanframe/global/assets/smarteditor/img/photoQuickPopup/btn_confirm.png");
    	 }
     }
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
	        ...
	        aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="/humanframe/global/assets/smarteditor/img/photoQuickPopup/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');
 		...
 		try{
	    	...
	    	for (var i = 0; i < aTemp.length ; i++){
	    		if( !!aTemp[i] && aTemp[i] != "" && aTemp[i].indexOf("=") > 0){
	    			...
	    			htTemp[aSubTemp[0]] = aSubTemp[1].split("|").join("=");
	    		}
	 		}
 		}catch(e){}
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/comm/editor/uploadFile',
 	        sCallback : '/humanframe/global/assets/smarteditor/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
...
// 	    		 alert("success");
 	    		...
 	    		aResult[0].sFileURL = aResult[0].sFileURL.toString().split("^").join("&");
 	    		setPhotoToEditor(aResult);
 	    		//버튼 비활성화
 	    		goReadyMode();
 	    		oFileUploader.reset();
